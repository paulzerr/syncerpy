...
class LimitationQueue(BaseQueue):
    ...
    def _wait_until_queue_filled(self, thr_fraction=0.5, check_interval=5.0):
        """
        Wait until the queue size is greater than
        `self.max_loaded * thr_fraction`.

        Args:
            thr_fraction:    (float) The fraction of `self.max_loaded` that
                                     the queue size should exceed.
            check_interval:  (float) Time in seconds to wait between checks.
        """
        target_qsize = int(self.max_loaded * thr_fraction)
        while self.loaded_queue.qsize() < target_qsize:
            logger.info("Queue size too low "
                        "(current: {}, target: {}). Waiting...".format(self.loaded_queue.qsize(),
                                                                       target_qsize)
            )
    ...
    def get_random_study(self):
        ...
        # Ensure the queue is filled before proceeding
        self._wait_until_queue_filled()
