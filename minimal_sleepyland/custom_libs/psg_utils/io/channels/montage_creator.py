...
import warnings
import numpy as np
from psg_utils.io.channels import (ChannelMontageTuple, ChannelMontage,
                                    infer_channel_types, assert_channel_types)
...
def auto_infer_referencing(channel_names, channel_types=None, types=("EEG",), on_already_ref="raise"):
    ...
    Channels must contain the appropriate MASTOIDS, otherwise a warning is raised and
    no referencing done.
    ...
    channel_names_montage = ChannelMontageTuple(channel_names, relax=True)
    inferred_types = infer_channel_types(channel_names_montage)
