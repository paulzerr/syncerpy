...
EEG_CHANNELS_CENTRAL = list(filter(lambda chan: chan in ('C5', 'C3', 'C1', 'Cz', 'C2', 'C4', 'C6'),
                           make_standard_montage("standard_1020").ch_names))
EEG_REGEX_CENTRAL = re.compile(r"(\bEEG|\b{})".format(r"|\b".join(EEG_CHANNELS_CENTRAL)), re.IGNORECASE)

VALID_CHANNEL_TYPES = ("EEG", "EOG", "EMG", "MASTOID", "OTHER")

def is_eeg_central(channel_name):
    if not isinstance(channel_name, ChannelMontage):
        channel_name = ChannelMontage(channel_name, relax=True)
    return bool(re.search(EEG_REGEX_CENTRAL, channel_name.channel))
