...
from psg_utils.io.channels import infer_channel_types, is_eeg_central
...
def get_argparser():
    ...
    parser.add_argument("--data_per_prediction", type=int, default=None,
                        help='Number of samples that should make up each sleep'
                             ...
                             'signal.')
    parser.add_argument("--eeg_channels", nargs='*', type=str, default=None,
                        help="A list of eeg channels to use instead of those "
                             "specified in the parameter file.")
    parser.add_argument("--eog_channels", nargs='*', type=str, default=None,
                        help="A list of eog channels to use instead of those "
                             "specified in the parameter file.")
    ...
    parser.add_argument("--dataset", type=str, default=None,
                        help="Specify the Dataset of those stored in the hparams (mainly nsrr list)"
                             "file that prediction should be performed on. Needed if --folder_regex is set. "
...
def get_datasets(hparams, args):
    ...
    if args.folder_regex:
        ...
        # We load the dataset hyperparameters of the associated dataset if exist
        # OR We load one of those specified in
        # the stored hyperparameter files and use it as a guide for how to
        # handle this new, undescribed dataset
        try:
            dataset_hparams = all_dataset_hparams[args.dataset]
        except Exception as e:
            # If any error occurs, catch it and continue with the next iteration
            print(f"An error occurred with: {e}\n"
                  f"The dataset in the folder regex {args.dataset} is not in the nsrr list.\n"
                  f"Loading one of those specified in the nsrr list...")
            dataset_hparams = all_dataset_hparams["external"]
...
def run_pred_on_pair(sleep_study_pair, seq, model, model_func, out_dir, channel_sets, args):
    ...
    for k, (sub_folder_name, channels_to_load) in enumerate(channel_sets):
        ...
        channel_names = list(channels_to_load)
        ...
        if args.model_external=="yasa" and "EEG" in channel_types and not is_eeg_central(channel_names[0]):
            logger.info(f"Skipping (channels={channels_to_load}) - YASA run on central derivations.")
            k_ext_model += 1
            continue
        ...
        if args.model_external == "pops" and only_one_eeg and not is_eeg_central(channel_names[0]):
            logger.info(f"Skipping (channels={channels_to_load}) - POPS run on EEG only and central derivations.")
            k_ext_model += 1
            continue
...
def run_pred(dataset,
    ...
    for i, sleep_study_pair in enumerate(dataset):
        ...
        if hparams['select_eeg_channels'] or hparams['select_eog_channels']:
            selected_eeg = set(hparams['select_eeg_channels']) if hparams['select_eeg_channels'] else set()
            selected_eog = set(hparams['select_eog_channels']) if hparams['select_eog_channels'] else set()

            def is_valid_channel(item):
                channels = set(item[1])
                eeg_match = selected_eeg.intersection(channels) if selected_eeg else True
                eog_match = selected_eog.intersection(channels) if selected_eog else True
                return eeg_match and eog_match

            channel_sets = [item for item in channel_sets if is_valid_channel(item)]
...
def run(args):
    ...
    hparams["select_eeg_channels"] = args.eeg_channels
    hparams["select_eog_channels"] = args.eog_channels
    hparams["channel_sampling_groups"] = None
    logger.info(f"Evaluating using eeg channels {args.eeg_channels}"
                f"and eog channels {args.eog_channels}")
