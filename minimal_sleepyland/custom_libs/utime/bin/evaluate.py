...
def get_argparser():
    ...
    """
    parser = ArgumentParser(description='Evaluate a U-Time-Sleepyland model.')
...
def predict_on(study_pair, seq, model=None, model_func=None, model_external=None,
    ...
    if not model_external:
        ...
        if model_external == 'yasa':
            ...
            channel_names = study_pair.select_channels.names
            channel_types = infer_channel_types(channel_names)

            # Create a raw object to give in input to yasa classifier
            info_ = mne.create_info(ch_names=channel_names,
                                   ch_types=[ch.lower() for ch in channel_types],
                                   sfreq=study_pair.sample_rate)
            raw_ = mne.io.RawArray(study_pair.psg.T, info_)

            # Identify EEG and EOG channels based on inferred types
            eeg_name = next((name for name, ch_type in zip(channel_names, channel_types) if ch_type.lower() == 'eeg'),
                            None)
            eog_name = next((name for name, ch_type in zip(channel_names, channel_types) if ch_type.lower() == 'eog'),
                            None)
            emg_name = next((name for name, ch_type in zip(channel_names, channel_types) if ch_type.lower() == 'emg'),
                            None)
        ...
        if model_external == 'pops':
            ...
            sig = study_pair.psg
            ch_eeg = study_pair.select_channels.names[0]
            # d = pd.read_table('d.txt', header=None)

            p = proj.empty_inst(study_pair.identifier,
                                int(sig.shape[0]/study_pair.sample_rate), 1)

            p.insert_signal(ch_eeg, sig, study_pair.sample_rate)
...
def get_sequencer(dataset, hparams):
    ...
    seq = get_batch_sequence(dataset_queue=dataset_queue,
                             ...
                             batch_wise_scaling=bool(hparams.get("scaler", None)),
                             **hparams["fit"],
                             no_log=False,
