    ...
    def __init__(self,
        ...
        Args:
            ...
            batch_scaler:      (string) The name of a sklearn.preprocessing
                                        ...
                                        batch (optional)
            get_spectrogram:   (str)    If not None the spectrogram of the PSG data
                                        will be computed and returned in the
                                        batch. If "both", the spectrogram will
                                        be returned as a separate element in the
                                        batch. If "replace", the raw data will be
                                        replaced by the spectrogram in the batch.
    ...
    def process_batch(self, X, y):
        ...
        Processing includes:
          ...
          4) Ensures both X and y has a 'batch dimension', even if batch_size
             is 1.
          5) if self.get_spectrogram is set, computes the spectrogram of X
          6) If a 'batch_scaler' is set, scales the X data
          7) Performs augmentation on the batch if self.augmenters is set and
        ...
        if self.get_spectrogram:
            ...
            if self.get_spectrogram == "both":  # xsleepnet
                ...
                if self.augmentation_enabled:
                    ...
                    self.augment(spectrogram, y, w=w)
                X = [X, spectrogram]
                ...
                if self.augmentation_enabled:
                    self.augment(X, y, w=w)
                if np.iscomplex(X).any() or np.isnan(X).any() or np.isinf(X).any():
                    logger.info("Augmented/scaled spectrogram contains unexpected values. ")
            else:
                raise ValueError("Invalid value for 'get_spectrogram'. " 
